...
public class OpenEdgeDatabaseModelFactory(IDiagnosticsLogger<DbLoggerCategory.Scaffolding> logger) : IDatabaseModelFactory
{
    ...
    public DatabaseModel Create(DbConnection connection, DatabaseModelFactoryOptions options)
    {
        ...
        var connectionStartedOpen = connection.State is ConnectionState.Open;
        ...
        DatabaseModel databaseModel)
    {
        ...
        var commandText = @"SELECT t.""_File-Name"" AS 'name' FROM ""pub"".""_File"" t ";
        var filter = $"WHERE t.\"_File-Name\" <> '{HistoryRepository.DefaultTableName}' {(tableFilter != null ? $" AND {tableFilter("t.\"_file-name\"")}" : "")}";
        ...
        DatabaseModel databaseModel)
    {
        ...
        var tableColumnGroups = reader.Cast<DbDataRecord>()
            .GroupBy(ddr => ddr.GetValueOrDefault<string>("name"));
